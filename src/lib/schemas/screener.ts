...
export const screenerSubmissionSchema = z
  .object({
    ...
    context: z.record(z.string(), z.string().max(300)),
    profile: z.record(z.string(), z.string().max(300)).optional().default({}),
    answers: z
      .array(dimensionAnswerSchema)
      .min(10, "Responda todas as 10 dimensões")
      .max(10, "Responda apenas as 10 dimensões"),
    commercialAnswer: z.string().max(500).optional().default(""),
    company: z
      .object({
...
// --- Agent payload schema (legacy/duplicado — o worker usa @/lib/schemas/agent-payload) ---
